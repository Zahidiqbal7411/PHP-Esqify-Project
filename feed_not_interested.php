<?php
<<<<<<< HEAD
require_once 'connection.php'; // loads $conn and addNotification()
...
<<<<<<< HEAD
    ...
    $checkStmt = $conn->prepare("SELECT * FROM feed_not_interested WHERE feed_id = ? AND user_id = ? LIMIT 1");
    $checkStmt->execute([$feedId, $userId]);
=======
    // Check if already marked as not interested
...
<<<<<<< HEAD
        $deleteStmt = $conn->prepare("DELETE FROM feed_not_interested WHERE id = ?");
        $deleteStmt->execute([$existing['id']]);
        echo json_encode(['status' => 'interested']);
    } else {
        // Mark as "not interested"
        $insertStmt = $conn->prepare("INSERT INTO feed_not_interested (feed_id, user_id, created_at) VALUES (?, ?, NOW())");
        $insertStmt->execute([$feedId, $userId]);
=======
