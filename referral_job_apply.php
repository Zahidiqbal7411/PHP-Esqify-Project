...
// ===== Database Connection =====
require_once 'connection.php'; // this defines $conn
...
try {
    ...
    $stmt = $conn->prepare("SELECT * FROM eq_jobs WHERE id = :job_id AND LOWER(job_type) = 'referral' LIMIT 1");
    ...
    $check = $conn->prepare("SELECT id FROM job_applications WHERE applicant_id = :user_id AND job_id = :job_id");
